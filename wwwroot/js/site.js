...
window.dragDrop = {
    init: function (elementId, dotNetHelper) {
        ...
        element.addEventListener('touchstart', function (event) {
            const touch = event.touches[0]; dotNetHelper.invokeMethodAsync('OnMouseDown', touch.clientX, touch.clientY);
        });
        element.addEventListener('touchmove', function (event) {
            const touch = event.touches[0]; dotNetHelper.invokeMethodAsync('OnMouseMove', touch.clientX, touch.clientY);
        });
        element.addEventListener('touchend', function (event) {
            const touch = event.changedTouches[0]; dotNetHelper.invokeMethodAsync('OnMouseUp', touch.clientX, touch.clientY);
        });
    },
}